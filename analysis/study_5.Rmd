...
```{r load-data}
data <- read.csv("../data/study-5/non_anonymized/study-5_data.csv")
...
```{r}
second_time_summary <- main_data_filtered %>%
  group_by(partner_status, partner_first_choice, successful_first_time) %>%
  tidyboot_mean(participant_second_choice_generous, na.rm = T)
```
...
ggplot(second_time_summary, aes(x = partner_first_choice, y = empirical_stat, color = partner_status)) +
  ...
  scale_y_continuous(limits = c(0, 1)) +
  scale_color_brewer(palette = "Dark2") +
  facet_wrap(~ successful_first_time, 
             labeller = "label_both")
```

Plot partner first choices
...
tidy_model <- tidy(model2, effects = "fixed", conf.int = TRUE)
# Relabel terms for plot clarity
tidy_model <- tidy_model %>%
  filter(term %in% c(
    "(Intercept)",
    "partner_first_choice1",
    "participant_first_choice1",
    "partner_first_choice1:participant_first_choice1"
  )) %>%
  mutate(term = recode(term,
    `(Intercept)`                                     = "Intercept",
    `partner_first_choice1`                          = "Simple reciprocity",
    `participant_first_choice1`                      = "Habit",
    `partner_first_choice1:participant_first_choice1`= "Heuristic learning"
  )) %>% 
...
ggplot(tidy_model, aes(x = estimate, y = term)) +
  ...
  lims(x = c(-1.1, 0.4)) + 
  labs(
    title = "Study 5: Generous second choice",
    x = "Estimate (log-odds)",
    y = "Predictor"
  ) 

ggsave(
  here("figures/study-5-second-choice-model.pdf")
)
```
```{r}
library(ggeffects)
...
preds <- ggpredict(model2, terms = c("partner_first_choice", "partner_status"))

# Plot predicted probabilities
plot(preds) +
  labs(
    title = "Predicted Probability of Choosing 'Give' (Second Interaction)",
    x = "Partner's First Choice",
...
```{r}
first_time_coord <- 
  main_data_filtered %>% filter(successful_first_time == TRUE) %>%
  group_by(partner_status) %>%
  tidyboot_mean(participant_first_choice_generous, na.rm = T) %>%
  mutate(partner_first_choice = "first_time",
         did_coordinate = "yes")
...
second_time_coord <- 
  ...
  group_by(partner_status, partner_first_choice) %>%
  tidyboot_mean(participant_second_choice_generous, na.rm = T) %>%
  mutate(did_coordinate = "yes")

first_time_miscoord <- 
  main_data_filtered %>% filter(successful_first_time == FALSE) %>%
  group_by(partner_status) %>%
  tidyboot_mean(participant_first_choice_generous, na.rm = T) %>%
  mutate(partner_first_choice = "first_time",
         did_coordinate = "no")

second_time_miscoord <-
  main_data_filtered %>% filter(successful_first_time == FALSE) %>%
  group_by(partner_status, partner_first_choice) %>%
  tidyboot_mean(participant_second_choice_generous, na.rm = T) %>%
  mutate(did_coordinate = "no")
```

Join first and second time; columns are `partner_status`, `partner_first_choice` ('first_time' if its the first time), and successful_first_time.

```{r, include=FALSE}
coord_all <- bind_rows(
  first_time_coord, second_time_coord, first_time_miscoord, second_time_miscoord
)
```

```{r}
ggplot(coord_all, aes(x = partner_first_choice, y = empirical_stat, color = partner_status)) +
  geom_point(size = 3, position = position_dodge(width = 0.3)) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), size = 1, 
                width = 0.2, position = position_dodge(width = 0.3)) +
  labs(title = "Study 5", x = "Observed partner first choice",
       y = "Proportion choosing to give") +
  scale_color_brewer(palette = "Dark2") +
  scale_y_continuous(limits = c(0, 1)) +
  facet_wrap(~ did_coordinate, labeller = "label_both") +
