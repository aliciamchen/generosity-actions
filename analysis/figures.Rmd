...
# Per-subject means for spaghetti lines
# Compute dodged x positions to match violin dodge (width = 0.7, 3 groups)
add_dodge_x <- function(df, x_var = "relationship", dodge_var = "next_interaction", width = 0.7) {
  n_groups <- length(unique(df[[dodge_var]]))
  df %>%
    mutate(
      x_num = as.numeric(.data[[x_var]]),
      dodge_i = as.numeric(.data[[dodge_var]]),
      x_dodge = x_num + width * (dodge_i - (n_groups + 1) / 2) / n_groups
    )
}

study_1_subject_means <- study_1_data %>%
  group_by(subject_id, relationship, next_interaction) %>%
  summarize(mean_rating = mean(likert_rating, na.rm = TRUE), .groups = "drop") %>%
  add_dodge_x()
```


```{r fig.width=6, fig.height=3.5, dpi=125}
study_1_rel_levels <- levels(study_1_data$relationship)

ggplot(
  study_1_data %>% add_dodge_x(),
  aes(x = x_dodge, y = likert_rating, fill = next_interaction)
) +
  geom_line(
    data = study_1_subject_means,
    aes(x = x_dodge, y = mean_rating, group = interaction(subject_id, relationship)),
    alpha = 0.13, linewidth = 0.3, inherit.aes = FALSE
  ) +
  geom_violin(
    aes(group = interaction(relationship, next_interaction)),
    bw = 0.43,
    alpha = 0.5,
    width = 0.7 / 3 * 1.2
  ) +
  geom_crossbar(
    data = study_1_summary %>% add_dodge_x(),
    aes(
      x = x_dodge,
      y = empirical_stat,
      ymin = ci_lower,
  ...
  scale_x_continuous(breaks = seq_along(study_1_rel_levels), labels = study_1_rel_levels) +
...
ggsave(here("figures/study-1_main.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-1_scenarios.pdf"), device = cairo_pdf)
```
...
study_2_subject_means <- study_2_data %>%
  group_by(subject_id, relationship, next_interaction) %>%
  summarize(mean_rating = mean(likert_rating, na.rm = TRUE), .groups = "drop") %>%
  add_dodge_x()
```
...
```{r fig.width=6, fig.height=3.5, dpi=125}
study_2_rel_levels <- levels(study_2_data$relationship)

ggplot(
  study_2_data %>% add_dodge_x(),
  aes(x = x_dodge, y = likert_rating, fill = next_interaction)
) +
  geom_line(
    data = study_2_subject_means,
    aes(x = x_dodge, y = mean_rating, group = interaction(subject_id, relationship)),
    alpha = 0.13, linewidth = 0.3, inherit.aes = FALSE
  ) +
  geom_violin(
  ...
  geom_crossbar(
    data = study_2_summary %>% add_dodge_x(),
    aes(
      x = x_dodge,
      y = empirical_stat,
      ymin = ci_lower,
  ...
  scale_fill_manual(values = action_colors) +
  scale_color_manual(values = action_colors) +
  scale_x_continuous(breaks = seq_along(study_2_rel_levels), labels = study_2_rel_levels) +
  scale_y_continuous(breaks = c(1, 2, 3, 4, 5, 6, 7)) +
  labs(
...
ggsave(here("figures/study-2_main.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-2_scenarios.pdf"), device = cairo_pdf)
```
## Validation study
...
ggsave(here("figures/study-2_benefit.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-2_effort.pdf"), device = cairo_pdf)
```
...
study_3_subject_means <- study_3_data %>%
  group_by(subject_id, relationship, next_interaction) %>%
  summarize(mean_rating = mean(likert_rating, na.rm = TRUE), .groups = "drop") %>%
  add_dodge_x()
```
...
```{r fig.width=6, fig.height=3.5, dpi=125}
study_3_rel_levels <- levels(study_3_data$relationship)

ggplot(
  study_3_data %>% add_dodge_x(),
  aes(x = x_dodge, y = likert_rating, fill = next_interaction)
) +
  geom_line(
    data = study_3_subject_means,
    aes(x = x_dodge, y = mean_rating, group = interaction(subject_id, relationship)),
    alpha = 0.13, linewidth = 0.3, inherit.aes = FALSE
  ) +
  geom_violin(
  ...
  geom_crossbar(
    data = study_3_summary %>% add_dodge_x(),
    aes(
      x = x_dodge,
      y = empirical_stat,
      ymin = ci_lower,
  ...
  scale_fill_manual(values = action_colors) +
  scale_color_manual(values = action_colors) +
  scale_x_continuous(breaks = seq_along(study_3_rel_levels), labels = study_3_rel_levels) +
  scale_y_continuous(breaks = c(1, 2, 3, 4, 5, 6, 7)) +
  labs(
  ...
  theme(legend.position = "bottom")
ggsave(here("figures/study-3_main.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-3_scenarios.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-3_benefit.pdf"), device = cairo_pdf)
```
```{r fig.width = 7, fig.height=4}
...
ggsave(here("figures/study-3_effort.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-4_main.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-4_implicit.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-4_effort-first-time.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-4_benefit-first-time.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-4_first_second.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-2-4_first_second.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-3-4_first_second.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-3_prestige.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-3_dominance.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/dominance_vs_prestige.pdf"), device = cairo_pdf)
```
```{r}
...
ggsave(here("figures/study-4_dominance-vs-first.pdf"), device = cairo_pdf)
...
ggsave(here("figures/study-4_prestige-vs-first.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-5_first_time_scenarios.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-5_first_time_all.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-5_second_time_successful.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-5_second_time_unsuccessful.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-6_first_time_scenarios.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-6_first_time_all.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-6_second_time_successful.pdf"), device = cairo_pdf)
```
...
ggsave(here("figures/study-6_second_time_unsuccessful.pdf"), device = cairo_pdf)
```
